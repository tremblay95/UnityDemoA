...
{
    [CustomEditor(typeof(TransitionSequenceStrategy))]
public class TransitionSequenceStrategyEditor : Editor
{
    private ReorderableList stepsList;
    private readonly string stepsListName = "transitionSteps";

    private void OnEnable()
    {
        SerializedProperty stepsProp = serializedObject.FindProperty(stepsListName);
        ...
        {
            EditorGUI.LabelField(rect, "Transition Steps");
        };
        ...
        stepsList.onAddDropdownCallback = (Rect buttonRect, ReorderableList list) =>
        {
            ...
            var stepTypes = AppDomain.CurrentDomain.GetAssemblies()
                ...
                .Where(t => typeof(TransitionStep).IsAssignableFrom(t) && !t.IsAbstract);
    ...
    {
        var strategy = (TransitionSequenceStrategy)target;
